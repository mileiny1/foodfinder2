...
from rest_framework_simplejwt.views import TokenObtainPairView # Import view for obtaining JWT tokens
...
from .serializers import RegisterSerializer, FoodSearchSerializer, LoginSerializer, RegisteredUserSerializer, UserProfileSerializer # Import serializers for user registration and food search
from .models import FoodSearch, RestaurantResult, UserProfile # Import models for food search and restaurant results
...
def register(request): # Define a view for user registration
    ...
    return Response(RegisteredUserSerializer(users).data, status=status.HTTP_201_CREATED)
    ...
    return Response(
        {
            ...
            "preferred_language": [
                {"value": value, "label": label}
                for value, label in UserProfile.PREFERRED_LANGUAGE_CHOICES
            ],
            "field_meta": {
                "birthday": {
                    "type": "date",
...
def my_profile(request):
    ...
    if request.method == 'GET':
        return Response(RegisteredUserSerializer(request.user).data, status=status.HTTP_200_OK)
